...
# import cpn_eval
from cpn_eval import EvaluatorCPN, load_jsonl, load_json
from stats_dwie.results.utils.print_tables_latex import get_csv_line_v2, print_latex_table_coreflinker, \
...
if __name__ == "__main__":
    ...
    if has_to_load:
        ...
        for (dirpath, dirnames, filenames) in os.walk(path_results, followlinks=True):
            ...
            for filename in filenames:
                ...
                if filename == config_file:
                    cnt += 1
                    if cnt % 50 == 0:
                        print('processed ', cnt, ' experiment executions')
                    print('=========PROCESSING=======', test_path)
                    try:
                        loaded_predicted_test = load_jsonl(test_path, None)
                    except:
                        print('!!!!!!!!error in loading the following test path: ', test_path)
                        continue
                    # loaded_predicted_test = load_jsonl(test_path, None)
                    ...
                    if len(loaded_gold_test) != len(loaded_predicted_test):
                        print('WARN: differences between predicted (', len(loaded_predicted_test), ') and gold (',
                              len(loaded_gold_test), ') for follwing dir: ', dirpath)
                        continue
        ...
        #     pickle.dump(config_to_results, handle, protocol=pickle.HIGHEST_PROTOCOL)
    else:
    ...
    # csv_title_line = get_csv_line({}, {}, '', only_f1=False, only_title=True)
    ...
    print('=================FROM NOW ON SHOULD BE FAST==================')
    ...
    for key_config, res_values in config_to_results.items():
        ...
        if 1 < len(res_values['experiments'].keys()):
            print('------WARNING TWO EQUAL EXPERIMENTS: ', res_values['experiments'].keys())
        ...
        for curr_experiment in res_values['experiments'].keys():
            ...
            print('CURR EXPERIMENT IS OF: ', curr_experiment)
            # if '20200618_coref_with_att_' in curr_experiment:
            #     print('FOUND WHAT YOU WERE LOOKING FOR: ', curr_experiment)
            # 20200524_coref_only
            has_to_exclude = False
            for curr_exclude in to_exclude:
            ...
            if has_to_exclude:
                print('=========excluding from cached=========', curr_experiment)
                continue

            # ex of curr_experiment: '20200522b'
            rels_mention_f1 = []
            rels_mention_pr = []
            rels_mention_re = []
            ...
            # the nr of experiment has to be 5, if less, ignore,
            # if len(rels_mention_f1) != 5 and not curr_experiment in {'20200530a'}:  # TODO: here see how to do for experiment with mention-based relations!
            # if len(rels_mention_f1) != 5:  # TODO: here see how to do for experiment with mention-based relations!

            if len(rels_mention_f1) < 1:  # TODO: here see how to do for experiment with mention-based relations!
                print('!!!!!!!!!!!!!!!!!!!IGNORING EXPERIMENT ', curr_experiment,
                      ' with len(rels_mention_f1) of ', len(rels_mention_f1), '!!!!!!!!!!!!!!!!')
            ...
            # curr_csv_line = get_csv_line(experiment_config=curr_experiment_config,
    ...
    print('==========================AND NOW PRINTING FOR NILL==========================')
