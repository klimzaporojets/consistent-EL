...
class EntityEmbbederKB(nn.Module):
    ...
    def __init__(self, dictionaries, config):
        ...
        logger.info('EntityEmbbederKB: %s %s ' % (len(self.dictionary), len(self.preds)))
    ...
    def load_triples(self, filename):
        logger.info('loading triples: %s' % filename)
        maxsize = 0
        self.preds = {}
        self.objs = {}
        ...
        logger.info('maximum size: %s' % maxsize)
    ...
    def forward(self, candidates):
        ...
        inputs_x = torch.LongTensor([mapping2[candidate_idx] for candidate_idx in candidates.view(-1).tolist()]).view(
            candidates.size()).to(candidates.device)
