...
from modules.misc.misc import batched_index_select, bucket_values
...
def inspect(name, x):
    ...
    print('{}\tnorm: {} - {}'.format(name, x_norm.min().item(), x_norm.max().item()))
    print('{}\tdata: {}'.format(name, x_norm))
...
def predict_scores_mtt(scores, linker_spans, linker_candidates, candidate_lengths, entity_dictionary: Dictionary):
    ...
    for batch_idx, span_idx in torch.nonzero(candidate_lengths).tolist():
        ...
        # UNCOMMENT if also want to add the root score, not sure if it is needed
        # c_ = ['ROOT'] + c_
        # # + 1 because of root candidate
        # s_ = scores[batch_idx, span_idx, :num_candidates + 1].tolist()
...
# def prune_spans_hoi(span_scores, sequence_lengths, sort_aft_pruning, prune_ratio=0.2, no_cross_overlap=False):
#     # this one works without batch dimension
#     # span_scores.shape -->
#     #
#     span_lengths = (sequence_lengths.float() * prune_ratio + 1).long()
#     span_scores = span_scores.view(-1)
#
#     values, top_indices = torch.topk(span_scores, span_lengths.max().item(), largest=True, sorted=True)
#
#     # span_scores.shape --> [1,1440]
...
def filter_spans(span_vecs, span_indices):
    ...
    # print('filter_spans span_vecs.shape is ', span_vecs.shape)
    # print('filter_spans tmp.shape is ', tmp.shape)
    # print('filter_spans span_indices.shape is ', span_indices.shape)
    # print('filter_spans span_indices value is ', span_indices)
...
# def filter_spans_hoi(span_vecs, span_indices):
#     """
#     This function ....
#     :param span_vecs:
#     :param span_indices:
...
#     # (kzaporoj) returns a vector of [1, 9, 1676] where 9 is the number of spans and 1676 is the dimension
#     # span_indices are tensor([[ 16,  27, 207, 227, 245, 250, 256, 266, 285]])
#     return index_select(tmp[0], span_indices[0]).unsqueeze(0)
...
def indices_to_spans(top_indices, span_lengths, max_span_width):
    # this is going to be deprecated, this is why was changed by // ,
    # others to check: true_divide : but this one returns the torch tensor float type
...
def overwrite_spans_hoi(span_vecs, span_pruned_indices, span_lengths, values):
    ...
    if length > 0:
        # indices = span_pruned_indices[0, :length]
        # output[0, indices, :] = values[0, :indices.size(0), :]
...
def span_distance_tokens(span_begin, span_end):
    ...
    # for a,b in [(53, 11),(11,53),(11,11),(11,12)]:
    #     print('a:', span_begin[0,a].item(), span_end[0,a].item())
    #     print('b:', span_begin[0,b].item(), span_end[0,b].item())
    #     print('dist[a,b]:', span_dist[0,a,b].item())
...
class SpanPairs(nn.Module):
    ...
    def get_distance_embedding(self, span_begin, span_end):
        # print('in distance embedding, span_begin device: ', span_begin.device, ' span_end device: ', span_end.device)
        span_dist = self.distance_function(span_begin, span_end)
        span_dist = bucket_values(span_dist, num_total_buckets=self.num_distance_buckets)
